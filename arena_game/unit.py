...
class BaseUnit(ABC):
    ...
    def stamina_points(self):
        """ возвращаем аттрибут выносливости  в красивом виде """
        return round(self.stamina, 1)

    def equip_weapon(self, weapon: Weapon):
        """ присваиваем нашему герою новое оружие """
        self.weapon = weapon
        return f"{self.name} экипирован оружием {self.weapon}"
    ...
    def _count_damage(self, target: BaseUnit) -> int:
        ...
        damage = self.weapon.damage * self.unit_class.attack  # расчета урона игрока
        if target.stamina >= target.armor.stamina_per_turn * target.unit_class.stamina:
            target.stamina -= target.armor.stamina_per_turn * target.unit_class.stamina  # уменьшение выносливости защищающегося при использовании брони
            damage -= target.armor.defence * target.unit_class.armor
        damage = round(damage, 1)
        target.get_damage(damage)  # цель получает урон
        return damage  # возвращаем предполагаемый урон для последующего вывода пользователю в текстовом виде
